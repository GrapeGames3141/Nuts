...
func _init() -> void:
    ...
    assert(is_zero_approx(ad_service.game_content_reserve_height()))
    assert(is_zero_approx(ad_service.banner_height()))
    assert(is_zero_approx(ad_service.native_banner_height()))
    var ad_service_source := FileAccess.get_file_as_string("res://autoload/ad_bar_service.gd")
    var ad_contract_game_source := FileAccess.get_file_as_string("res://scripts/game.gd")
    assert(ad_service_source.contains("func native_banner_height()") and ad_service_source.contains("func game_content_reserve_height()") and ad_service_source.contains("return 0.0") and ad_service_source.contains("func banner_height()") and ad_service_source.contains("return game_content_reserve_height()"))
    assert(not ad_service_source.contains("AdDisclosure") and not ad_service_source.contains("AdBarOverlay") and not ad_service_source.contains("_ensure_disclosure") and not ad_service_source.contains("_layout_disclosure") and not ad_service_source.contains("DISCLOSURE_HEIGHT"))
    assert(ad_contract_game_source.contains("game_content_reserve_height") and not ad_contract_game_source.contains("ad_service.call(\"banner_height\")"))
