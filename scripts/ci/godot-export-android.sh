#!/usr/bin/env bash
# Export Godot Android release AAB, and optionally a debug APK.
...
RELEASE_PACKAGE_NAME="${RELEASE_PACKAGE_NAME:-com.grapegames.nuts}"
DEBUG_PACKAGE_NAME="${DEBUG_PACKAGE_NAME:-com.grapegames.nuts.debug}"
ARTIFACT_STEM="${ARTIFACT_STEM:-Nuts}"
VERSION_CODE="${VERSION_CODE:-1}"
VERSION_NAME="${VERSION_NAME:-1.${VERSION_CODE}}"
ADMOB_ANDROID_APP_ID="${ADMOB_ANDROID_APP_ID:-${NUTS_ADMOB_ANDROID_APP_ID:-ca-app-pub-3940256099942544~3347511713}}"
...
install_godot() {
	local cache="$HOME/.cache/nuts/godot"
...
write_admob_config() {
	...
	# TODO(ads-live): restore the production banner secret when leaving closed test:
	# local nuts_android="${NUTS_ADMOB_ANDROID_BANNER_UNIT_ID:-${ADMOB_BANNER_UNIT_ID:-$sample_android}}"
	local sample_android="ca-app-pub-3940256099942544/6300978111"
	local sample_ios="ca-app-pub-3940256099942544/2435281174"
	local nuts_android="$sample_android"
	local nuts_ios="${NUTS_ADMOB_IOS_BANNER_UNIT_ID:-$sample_ios}"
	...
	cat >"$PROJECT/config/admob.json" <<EOF
{
  ...
  "products": {
    "nuts": {
      "android_banner_unit_id": "${nuts_android}",
      "ios_banner_unit_id": "${nuts_ios}"
    }
  },
...
install_admob_android_binaries() {
	...
	local cache="${ADMOB_CACHE_DIR:-$HOME/.cache/nuts/admob}"
...
standalone_bundletool() {
	...
	local cache_dir="${BUNDLETOOL_CACHE_DIR:-$HOME/.cache/nuts/bundletool}"
